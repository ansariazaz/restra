...
const CreateContainer = () => {
  ...
  const saveDetails = () => {
    setIsLoading(true)
    try {
      if ((!title || !calories || !categories || !price || !imageAsset)) {
        setFields(true)
  ...
  return (
    <div className='w-full h-auto min-h-screen flex items-center justify-center'>
      <div className='w-[90%] md:w-[75%] border border-gray-300 rounded-lg gap-4 p-4 flex flex-col items-center justify-center'>
        {fields && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0 }} exit={{ opacity: 0 }} className={`w-full p-2 rounded-lg text-center text-lg font-semibold ${alertStatus === "danger" ? "bg-red-400 text-red-800" : "bg-emerald-400 text-emerald-800"}`}>{msg}</motion.p>
        )}
